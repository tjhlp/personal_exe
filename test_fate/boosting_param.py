...
class BoostingParam(BaseParam):
    ...
    def check(self):
        ...
        if self.subsample_random_seed is not None:
            assert type(
                self.subsample_random_seed) == int and self.subsample_random_seed >= 0, 'random seed must be an integer >= 0'
...
class HeteroFastSecureBoostParam(HeteroSecureBoostParam):
    ...
    def __init__(self, tree_param: DecisionTreeParam = DecisionTreeParam(), task_type=consts.CLASSIFICATION,
                 ...
                 complete_secure=False, tree_num_per_party=1, guest_depth=1, host_depth=1, work_mode='mix',
                 metrics=None,
